// X11 backend for the screen locker: a full-screen override-redirect window
// that grabs the keyboard and pointer and auto-unlocks after a timeout.
package lock
...
// x11Locker is the X11 implementation of Backend.
...
// newX11 builds an X11 locker from cfg (defaults already applied by the caller).
func newX11(cfg Config) (*x11Locker, error) {
	return &x11Locker{
...
func (l *x11Locker) Name() string { return "x11" }
...
func (l *x11Locker) IsRunning() bool {
	l.mu.Lock()
	defer l.mu.Unlock()
	return l.running
}

func (l *x11Locker) createWindow() error {
	// Generate window ID
...
func (l *x11Locker) destroyWindow() {
	if l.window != 0 {
...
func (l *x11Locker) grabInputs() error {
	// Grab keyboard
...
func (l *x11Locker) ungrabInputs() {
	xproto.UngrabKeyboard(l.conn, xproto.TimeCurrentTime)
	xproto.UngrabPointer(l.conn, xproto.TimeCurrentTime)
}

func (l *x11Locker) runLoop() error {
...
func (l *x11Locker) processEvents() {
	for {
...
func (l *x11Locker) drawScreen(remaining time.Duration) {
